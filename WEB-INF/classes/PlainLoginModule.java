    ...
    */
    ...
    public boolean login() throws LoginException {
        ...
        try {
            ...
            FileInputStream fUsers = new FileInputStream(path + "/webapps/IDwebapp/META-INF/users/users");
            InputStreamReader iUsers = new InputStreamReader(fUsers);
            BufferedReader brUsers = new BufferedReader(iUsers);

            String lineUsers;
            while ((lineUsers = brUsers.readLine()) != null) {
                String user;
                String rolesString;
                user = lineUsers.split(":")[0];
                rolesString = lineUsers.split(":")[1]; //cv,cd,ca
                if(name.equals(user)){
                    roles = rolesString.split(",");
                    FileInputStream fShadow = new FileInputStream(path + "/webapps/IDwebapp/META-INF/users/shadow");
                    InputStreamReader iShadow = new InputStreamReader(fShadow);
                    BufferedReader brShadow = new BufferedReader(iShadow);

                    String lineShadow;
                    String passwordShadow = "";
                    String salt;
                    while ((lineShadow = brShadow.readLine()) != null) {
                        if(lineShadow.split(":")[0].equals(user)){
                            salt = lineShadow.split(":")[1];
                            passwordShadow = lineShadow.split(":")[2];
                        }
                    }
                    ...
                    try{
                        ...
                        if(!digestPassword.equals(passwordShadow)){
